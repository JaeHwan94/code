...
def Push_Data(data):
	...
	g = Gauge('Face_Member_Check_B',"Member's Face check processing time - B Group",registry=registry)
	g.set(data)

	push_to_gateway('http://192.168.2.1:9092',job='Face_Member_Checker_B',registry=registry)
...
def main():
	if request:
		...
		data['member_verify_time_b'] = m_data['member_verify_time']
		data['member_group'] = m_data['member_group']
		data['member_name'] = m_data['member_name']

		url = _url+"member-check:5000/b"
		re.post(url,headers=headers,data=json.dumps(data))
		
		Push_Data(data['member_verify_time_b'])
